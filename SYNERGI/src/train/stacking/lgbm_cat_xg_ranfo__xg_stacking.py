...
Meta model: XGBoost (shallow)  ← 과적합 방지형
"""
...
N_SPLITS = 5
N_CLASSES = 3
MODEL_NAME = "lgbm_xg_cat_ranfo__xg"

CSV_PATH = "/proj/home/ibs/spaceai_2025/ai2271056/SYNERGI/data/Illustris/Illustris_preprocess_SFR_no.csv"
OUT_BASE_MODEL = "/proj/home/ibs/spaceai_2025/ai2271056/SYNERGI/model/stacking/SFR_inf_remove"
OUT_BASE_EVAL  = "/proj/home/ibs/spaceai_2025/ai2271056/SYNERGI/evaluation/stacking/SFR_inf_remove"
...
# LOAD DATA
# ============================================
df = pd.read_csv(CSV_PATH, engine="python", sep=None, encoding="utf-8", quoting=csv.QUOTE_MINIMAL)
df.columns = [c.strip().strip("\ufeff") for c in df.columns]

label_col = [c for c in df.columns if c.strip().lower() == "phase"][0]
PHASE_MAP_ANY = {
    "-1": 1, -1: 1, "pre": 1,
     "0": 0,  0: 0, "non": 0,
     "1": 2,  1: 2, "post": 2
...
y_all = lab.map(PHASE_MAP_ANY).astype(int).to_numpy()
...
df, y_all = df.loc[nan_mask].reset_index(drop=True), y_all[nan_mask.values]
...
# ============================================
oof_pred = np.zeros((len(X_train), N_CLASSES * 4), dtype=float)
...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X_train, y_train), start=1):
    ...
    xgb = XGBClassifier(objective="multi:softprob", eval_metric="mlogloss", random_state=SEED, **best_params_xgb)
    xgb.fit(X_tr, y_tr)
    oof_pred[val_idx, 0:3] = xgb.predict_proba(X_val)

    lgbm = LGBMClassifier(random_state=SEED, **best_params_lgbm)
    lgbm.fit(X_tr, y_tr)
    oof_pred[val_idx, 3:6] = lgbm.predict_proba(X_val)

    cat = CatBoostClassifier(loss_function="MultiClass", verbose=0, random_state=SEED, **best_params_cat)
    cat.fit(X_tr, y_tr)
    oof_pred[val_idx, 6:9] = cat.predict_proba(X_val)

    ranfo = RandomForestClassifier(random_state=SEED, n_jobs=-1, **best_params_ranfo)
    ranfo.fit(X_tr, y_tr)
    oof_pred[val_idx, 9:12] = ranfo.predict_proba(X_val)
...
# ============================================
xgb_full = XGBClassifier(objective="multi:softprob", eval_metric="mlogloss", random_state=SEED, **best_params_xgb).fit(X_train, y_train)
lgbm_full = LGBMClassifier(random_state=SEED, **best_params_lgbm).fit(X_train, y_train)
cat_full = CatBoostClassifier(loss_function="MultiClass", verbose=0, random_state=SEED, **best_params_cat).fit(X_train, y_train)
ranfo_full = RandomForestClassifier(random_state=SEED, n_jobs=-1, **best_params_ranfo).fit(X_train, y_train)

proba_test = np.hstack([
    xgb_full.predict_proba(X_test),
    lgbm_full.predict_proba(X_test),
    cat_full.predict_proba(X_test),
    ranfo_full.predict_proba(X_test)     # ⭐ RF 포함
])
...
with open(os.path.join(eval_dir, "stacking_classification_report.txt"), "w") as f: f.write(report)
...
ranfo_path = os.path.join(model_dir, "ranfo_full.pkl")
meta_path = os.path.join(model_dir, "meta_xgb.pkl")

joblib.dump(xgb_full,  xgb_path)
joblib.dump(lgbm_full, lgbm_path)
joblib.dump(cat_full,  cat_path)
joblib.dump(ranfo_full, ranfo_path)
...
meta_json = {
    ...
    "classes": class_names,
    "oof": {"accuracy": round(oof_acc, 4), "macro_f1": round(oof_f1, 4)},
    "test": {"accuracy": round(test_acc, 4), "macro_f1": round(test_f1, 4)},
    "best_params": {
        "xgboost": best_params_xgb,
        "lightgbm": best_params_lgbm,
        "catboost": best_params_cat,
        "random_forest": best_params_ranfo
    },
    "model_paths": {
        "xgb": xgb_path,
        "lgbm": lgbm_path,
...
with open(os.path.join(model_dir, "meta.json"), "w") as f: json.dump(meta_json, f, indent=2)
