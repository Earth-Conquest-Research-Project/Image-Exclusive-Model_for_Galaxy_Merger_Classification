...
PROJECT_ROOTs = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../../.."))

DATA_DIR   = os.path.join(PROJECT_ROOTs, "data", "simulation_data")

MODEL_DIR = os.path.join(PROJECT_ROOT, "model", "classicalMachineLearning","SFR_inf_-1", MODEL_NAME)
EVAL_DIR  = os.path.join(PROJECT_ROOT, "evaluation", "classicalMachineLearning", "SFR_inf_-1", MODEL_NAME)
...
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../.."))
DATA_DIR_SIM = os.path.join(PROJECT_ROOTs, "data", "simulation_data")  # final_12 위치
DATA_DIR_ILL = os.path.join(PROJECT_ROOT,  "data", "Illustris")         # Illustris 위치

# 학습 데이터 (SFR=-1 들어있는 원본)
CSV_PATH = os.path.join(DATA_DIR_ILL, "Illustris_preprocess_SFR_-1.csv")

# ============================================================
# Load Base CSV (SFR 잘못된 파일)
# ============================================================
print(f"[INFO] Loading Illustris CSV: {CSV_PATH}")
df = pd.read_csv(CSV_PATH, engine="python")

# ============================================================
# Load final_12_datasetPhase_complete.csv (정확한 SFR)
# ============================================================
df_final = pd.read_csv(
    os.path.join(DATA_DIR_SIM, "final_12_datasetPhase_complete.csv"),
    engine="python"
)
df_final.columns = [c.strip().strip("\ufeff") for c in df_final.columns]

# ============================================================
# Replace BulgeMass
# ============================================================

df_final["_merge_key"] = df_final["Snapshot"].astype(str) + "_" + df_final["SubHaloID"].astype(str)
df["_merge_key"]       = df["Snapshot"].astype(str) + "_" + df["SubHaloID"].astype(str)

df_merged = df.merge(
    df_final[["_merge_key", "BulgeMass"]],
    on="_merge_key",
    how="left"
)

df_merged["BulgeMass"] = df_merged["BulgeMass_y"]  # df_final의 BulgeMass
df = df_merged.drop(columns=["_merge_key", "BulgeMass_y", "BulgeMass_x"])
...
exclude = {"Phase","SubHaloID", "Snapshot", "ID", "phase_5",
